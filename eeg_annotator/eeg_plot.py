...
class EEGPlot:
    ...
    def get_s_freq(self, raw_eeg):
        return raw_eeg.info["sfreq"]

    def get_signal_length_seconds(self, raw_eeg):
        s_freq = self.get_s_freq(raw_eeg)
    ...
    def read_excel(self, excel_path: str, s_freq: int = 256):
        ...
        mne_info = mne.create_info(
            ch_names=channel_pair_names, sfreq=s_freq, ch_types="eeg"
        )
        mne_data = mne.io.RawArray(eeg_df.values.T, info=mne_info, verbose=False)

        return (mne_data, signal_length_seconds)
    ...
    def read_edf(self, edf_path: str, differential_array):
        ...
        return (
            self.raw_to_bipolar(raw_eeg, differential_array),
    ...
    def raw_to_bipolar(self, raw_eeg, channel_pair_dict):
        ...
        channel_names_lowercase = [ch.lower() for ch in raw_eeg.info["ch_names"]]

        for key, value in channel_pair_dict.items():
            channel_a = value[0].strip()
            channel_b = value[1].strip()

            if channel_a.lower() in channel_names_lowercase:
                try:
                    c_1 = raw_eeg[channel_a][0][0].astype(np.float16)
                except ValueError:
                    channel_a = channel_a.upper()
                    c_1 = raw_eeg[channel_a][0][0].astype(np.float16)

                try:
                    c_2 = raw_eeg[channel_b][0][0].astype(np.float16)
                except ValueError:
                    channel_b = channel_b.upper()
                    c_2 = raw_eeg[channel_b][0][0].astype(np.float16)
